...
async function bundle(compiler, next) {
    ...
    if (!doBundle) {
        await compiler.addResource(path_1.resolve(cwd, input));
        return next();
    }
    let code = '';
    ...
    if (input === util_1.STDIN_FLAG && (code = code || util_1.dequote(await getStdIn(process.stdin)))) {
        compiler.stdinUsed = true;
        compiler.entrypoint = './__nexe_stdin.js';
        await compiler.addResource(path_1.resolve(cwd, compiler.entrypoint), code);
        return next();
    }
