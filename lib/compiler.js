...
class NexeCompiler {
    ...
    async readFileAsync(file) {
        ...
        if (!cachedFile) {
            ...
            cachedFile = {
                absPath,
                filename: file,
                contents: await util_1.readFileAsync(absPath, 'utf-8').catch((x) => {
                    if (x.code === 'ENOENT')
                        return '';
                    throw x;
                }),
    ...
    async _shouldCompileBinaryAsync(binary, location) {
        //SOMEDAY combine make/configure/vcBuild/and modified times of included files
        const { snapshot, build } = this.options;
        if (!binary) {
            return true;
        }
        if (build && snapshot != null && (await util_1.pathExistsAsync(snapshot))) {
            const snapshotLastModified = (await util_1.statAsync(snapshot)).mtimeMs;
            const binaryLastModified = (await util_1.statAsync(location)).mtimeMs;
    ...
    async compileAsync(target) {
        ...
        let binary = (await util_1.pathExistsAsync(location)) ? fs_1.createReadStream(location) : null;
    ...
    async _assembleDeliverable(binary) {
