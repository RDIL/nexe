...
async function disableNodeCli(compiler, next) {
    if (compiler.options.enableNodeCli) {
        return next();
    }
    if (util_1.semverGt(compiler.target.version, '11.6.0')) {
        await compiler.replaceInFileAsync('src/node.cc', /(?<!int )ProcessGlobalArgs\(argv[^;]*;/gm, '0;/*$&*/');
    }
        ...
        await compiler.replaceInFileAsync('src/node.cc', 'int i = 1; i < v8_argc; i++', 'int i = v8_argc; i < v8_argc; i++');
        let matches = 0;
        ...
        await compiler.replaceInFileAsync('src/node.cc', `${nodeccMarker} '-'`, 
